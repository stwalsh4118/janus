...
func TestNewCleanupService(t *testing.T) {
	...
	service := NewCleanupService(manager, timeout, interval, &logger)
...
func TestNewCleanupService_UsesDefaultLoggerWhenNil(t *testing.T) {
	manager := NewMemorySessionManager()
	timeout := 10 * time.Minute
	interval := 1 * time.Minute

	// Should not panic, should use default logger
	service := NewCleanupService(manager, timeout, interval, nil)
...
func TestCleanupService_StartStop(t *testing.T) {
	manager := NewMemorySessionManager()
	logger := zerolog.Nop()
	service := NewCleanupService(manager, 10*time.Minute, 100*time.Millisecond, &logger)
...
func TestCleanupService_RemovesInactiveSessions(t *testing.T) {
	...
	logger := zerolog.Nop()
	service := NewCleanupService(manager, timeout, 100*time.Millisecond, &logger)
